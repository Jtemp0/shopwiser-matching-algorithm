"""
Similarity self-tests for ``compute_similarity`` (fixtures for ``run_tests``).

Run via the CLI: ``uv run python main.py test-similarity``
...
def build_test_cases(tol_branded, tol_own, tol_unbranded):
    ...
    return [
        ...
        # v7: branded pass uses a relaxed pack check — same unit_value means same total weight,
        # so NaN vs known multipack is allowed (unit_value is the real guard).
        # In practice normalise.py stores total weight so a 6-pack 500ml/bottle would
        # have unit_value=3000ml (not 500ml), preventing a false-positive match.
        ('v5→v7: branded NaN-pack vs 6-pack with matching total unit → SHOULD match (relaxed branded)',
         _mk('still water', 3000, 'ml', 'Tesco', brand='evian', pq=None),
         _mk('still water', 3000, 'ml', 'ASDA',  brand='evian', pq=6),
         'branded', tol_branded, True),

        # Different total weight → rejected by unit_value check regardless of pack check
        ('v5: branded single 500ml vs 6-pack 3000ml total → must NOT match (unit mismatch)',
        ...
        ('v8: Decaff vs Decaffeinated → SHOULD match (synonym normalisation)',
         _mk('original decaff instant coffee',        200, 'g', 'Tesco', brand='nescafe'),
         _mk('original decaffeinated instant coffee', 200, 'g', 'ASDA',  brand='nescafe'),
         'branded', tol_branded, True),
        ...
        # ── v7: relaxed branded pack check (NaN vs known multipack) ──────────
        # Branded products often have pack_qty scraped on one side but NaN on the
        # other.  Unit values matching total weight is the real safeguard.
        ('v7: branded 5-pack 161g total vs NaN-pack 160g → SHOULD match (relaxed branded pack)',
         _mk('brunch bar peanut', 161.3, 'g', 'ASDA',  brand='cadbury', pq=5),
         _mk('brunch bar peanut', 160.0, 'g', 'Tesco', brand='cadbury', pq=None),
         'branded', tol_branded, True),

        ('v7: branded plant chocolate bar with vegan label → SHOULD match (vegan penalty reduced)',
         _mk('plant chocolate bar', 90.0, 'g', 'ASDA',  brand='cadbury', attrs=''),
         _mk('plant chocolate bar', 90.0, 'g', 'Sains', brand='cadbury', attrs='vegan'),
         'branded', tol_branded, True),

        # v7: own_brand NaN vs 6-pack must still be rejected (strict check preserved)
        ('v7: own_brand butter NaN-pack vs 6-pack 300ml → must NOT match (strict own_brand pack)',
         _mk('butter', 300.0, 'ml', 'ASDA',  brand=None, tier='standard', ptype='own_brand', pq=None),
         _mk('butter', 300.0, 'ml', 'Sains', brand=None, tier='standard', ptype='own_brand', pq=6),
         'own_brand', tol_own, False),
    ]
