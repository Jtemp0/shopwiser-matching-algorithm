"""
ShopWiser Clustering v3 — targeted quality fixes.

Key changes vs v2:
  1. Wine type: one-sided detection — penalty fires if EITHER side has a wine-type
     token and the other doesn't share it (was: only when BOTH sides have tokens).
  2. NAS/sugar-free: 'no added sugar', 'sugar free', 'zero sugar' treated as diet
     marker (same 0.10 penalty as 'diet').
  3. Milk fat type: mismatch between skimmed / semi-skimmed / whole / full-fat
     gets a 0.50 penalty (prevents skimmed vs semi-skimmed grouping).
  4. Short stripped name threshold: when BOTH stripped names (branded pass) have
     ≤3 tokens, require score ≥ 0.95 (prevents ham vs lamb stock cubes).
  5. Own-brand fuzzy threshold: 0.82 → 0.884 (rejects black beans vs black
     eyed beans, score 0.879 < 0.884).
  6. UNIT_TOLERANCE_OWN_BRAND: 0.03 → 0.05 (restores coverage for own-brand).
  7. FUZZY_THRESHOLD_NOUNIT: 0.90 → 0.88 (restores coverage for no-unit items).
"""
...
UNIT_TOLERANCE_BRANDED    = 0.03
UNIT_TOLERANCE_OWN_BRAND  = 0.05   # restored (was 0.03 in v2)
UNIT_TOLERANCE_UNBRANDED  = 0.05

FUZZY_THRESHOLD           = 0.82
FUZZY_THRESHOLD_OWNBRAND  = 0.884  # stricter for own-brand (was 0.82)
FUZZY_THRESHOLD_NOUNIT    = 0.88   # restored (was 0.90 in v2)

# When BOTH stripped names (branded) have ≤3 tokens, require very high similarity
SHORT_STRIPPED_THRESHOLD  = 0.95
...
ATTR_PENALTIES = {
    ...
    'fairtrade': 0.10,
    'vegan':     0.25,
    # 'diet' handled separately via _has_diet_marker() to include NAS variants
}
DIET_PENALTY = 0.10
...
# Fish/seafood preservation medium tokens
FISH_MEDIUM_TOKENS = {'brine', 'spring water', 'olive oil', 'sunflower oil', 'tomato', 'springwater'}

# Milk fat type patterns (checked in order — most specific first)
MILK_FAT_MAP = [
...
# NAS / sugar-free marker patterns
...
print(f'  Short stripped thresh: {SHORT_STRIPPED_THRESHOLD} (≤3 tokens, branded)')
...
def _attribute_penalty(attrs_a, attrs_b, name_a='', name_b='') -> float:
    ...
    # Standard keyword attributes (excluding 'diet' — handled separately)
    for attr, p in ATTR_PENALTIES.items():
        if (attr in str_a) != (attr in str_b):
            penalty += p

    # Diet / NAS marker (includes 'no added sugar', 'sugar free', etc.)
    if _has_diet_marker(str_a) != _has_diet_marker(str_b):
        penalty += DIET_PENALTY

    # Wine type — one-sided detection:
    # if EITHER side has a wine-type token and they don't fully agree → penalty
    ...
    # Milk fat type (skimmed vs semi-skimmed vs whole)
...
def compute_similarity(row_a, row_b, pass_type, unit_tolerance):
    ...
    # Short-name guard: only enforce when BOTH names are very short
    ...
    # Short stripped name: require very high similarity (branded pass, ≤3 tokens)
    if pass_type == 'branded':
...
tests = [
    # Baseline passing tests (carried over from v2)
    ...
    ('Knorr lamb stock cubes vs ham stock cubes → must NOT match',
     _mk('knorr stock cubes lamb', 104, 'g', 'Tesco', brand='knorr'),
     _mk('knorr stock cubes ham', 104, 'g', 'ASDA', brand='knorr'),
     'branded', UNIT_TOLERANCE_BRANDED, False),
    ...
    ('Malbec Rosé vs Malbec red wine → must NOT match (one-sided wine)',
     _mk('malbec rose wine', 750, 'ml', 'Tesco', brand=None, ptype='unbranded'),
     _mk('malbec red wine', 750, 'ml', 'ASDA', brand=None, ptype='unbranded'),
     'unbranded', UNIT_TOLERANCE_UNBRANDED, False),

    ('Malbec Rosé vs plain Malbec → must NOT match (one-sided wine token)',
     _mk('malbec rose', 750, 'ml', 'Tesco', brand=None, ptype='unbranded'),
     _mk('malbec', 750, 'ml', 'ASDA', brand=None, ptype='unbranded'),
     'unbranded', UNIT_TOLERANCE_UNBRANDED, False),
