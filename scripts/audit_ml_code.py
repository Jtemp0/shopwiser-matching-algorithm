...
ENS = Path('data/outputs/ensemble/ensemble_clusters_ml.csv')
SIZE_TOL = 0.15
...
def _size_mismatch(g: pd.DataFrame) -> float:  # smart-delta version
    uvs = g['unit_value'].dropna().values
    if len(uvs) < 2:
        return 0.0
    import numpy as np

    mx = float(np.max(np.abs(uvs)))
    mn = float(np.min(np.abs(uvs)))
    if mx < 1e-6:
        return 0.0
    return (mx - mn) / mx
